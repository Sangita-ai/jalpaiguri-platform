...
import { updateSimulatedSensors } from '../services/sensor.service';

export function initSensorBroadcast(io: SocketServer) {
  ...
  setInterval(async () => {
    try {
      ...
      const drainAlerts = await prisma.drainSensor.findMany({
      ...
      const waterAlerts = await prisma.waterSensor.findMany({
      ...
      const [drainSummary, waterSummary] = await Promise.all([
        prisma.drainSensor.groupBy({ by: ['status'], _count: { id: true } }),
        prisma.waterSensor.groupBy({ by: ['status'], _count: { id: true } }),
      ]);
  ...
  (global as any).broadcastComplaintUpdate = (complaint: any) => {
