...
export async function resolveWardFromGPS(lat: number, lng: number): Promise<string | null> {
...
export async function generateComplaintNumber(): Promise<string> {
  ...
  const count    = await prisma.complaint.count({ where: { submittedAt: { gte: startOf } } });
...
export async function getSummaryStats() {
  ...
  const [
    total, pending, resolved, slaBreaches,
    byCategory, prevMonthTotal, drainAlerts,
    avgResArr, activeWorkers,
  ] = await Promise.all([
    prisma.complaint.count({ where: { submittedAt: { gte: ninetyDaysAgo } } }),
    prisma.complaint.count({ where: { status: { notIn: ['RESOLVED','CLOSED'] }, submittedAt: { gte: ninetyDaysAgo } } }),
    prisma.complaint.count({ where: { status: { in: ['RESOLVED','CLOSED'] }, submittedAt: { gte: ninetyDaysAgo } } }),
    // SLA breach: submitted >48h ago, still not resolved
    prisma.complaint.count({ where: { status: { notIn: ['RESOLVED','CLOSED'] }, submittedAt: { lte: new Date(Date.now() - 48 * 3600000) } } }),
    prisma.complaint.groupBy({ by: ['category'], _count: { id: true }, where: { submittedAt: { gte: ninetyDaysAgo } } }),
    prisma.complaint.count({ where: { submittedAt: { gte: sixtyDaysAgo, lt: thirtyDaysAgo } } }),
    prisma.drainSensor.count({ where: { status: { in: ['OVERFLOW_RISK','OVERFLOW','HIGH'] } } }),
  ...
  const currentMonthTotal = await prisma.complaint.count({ where: { submittedAt: { gte: thirtyDaysAgo } } });
...
export async function getCategoryTrend(days: number) {
  const since = new Date(Date.now() - days * 86400000);
  const rows = await prisma.$queryRaw<Array<{ day: Date; submitted: bigint; resolved: bigint }>>`
    SELECT
...
export async function getSLAReport() {
  const ninetyDaysAgo = new Date(Date.now() - 90 * 86400000);
  const slaConfig = await prisma.slaConfig.findMany();
  ...
  const results = await Promise.all(slaConfig.map(async (sla) => {
    ...
    const onTime = await prisma.complaint.count({
      where: {
